...
// JSONP load (bypasses CORS on GitHub Pages)
async function loadFromBackend(){
  ...
  try{
    ...
    if(payload){
      ...
      state.coordMessage = payload.coordMessage || payload.coordMessageHtml || state.coordMessage || "";
      hydrateUI();
      console.log("[LOAD] from Sheets ok:", key);
      return payload;
    }else{
      console.log("[LOAD] nothing saved yet for:", key);
      return null;
    }
  }catch(err){
    console.warn("⚠️ Backend load falhou, mantendo local:", err);
    return null;
  }
}

// Cross-origin POST via hidden iframe avoids CORS and URL-length limits.
function saveToBackend(){
  ...
  try{
    ...
    console.log("[SAVE] posted to Sheets ok:", payload.key);
    toast("Salvo ✅");
  }catch(err){
